...
const MainSection = (() => {
  ...
  return (
    <div  id="mainSection" className='relative  flex flex-col width space-up '>
      {theme !== 8 && <MovingBalls />}
      <Link to="/about" className={`z-20 text-sec-color uppercase font-semibold text-xl ${theme == 9 ? "cursor-lobsterHover" : "cursor-pointer"} z-10 w-fit trans hover:opacity-90`}>Hello, My name is Mahmoud</Link>
      <h1 className={`z-20  font-bold ${theme == 9  ? "title-font-lobster " : "title-font"} text-rare-color text-[110px] max-[500px]:text-[75px] -mt-4 max-[500px]:mt-0 z-0`}>
        I build <span className="max-[500px]:hidden">websites.</span></h1>
        <h1 className={`z-20 max-[500px]:block hidden font-bold ${theme == 9  ? "title-font-lobster" : "title-font"} text-rare-color text-[75px] -mt-10 z-0`}>websites.</h1>
